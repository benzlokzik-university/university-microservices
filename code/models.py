...
class AddGameRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Описание игры", max_length=2000
    )
    min_players: int = Field(
        ..., description="Минимальное количество игроков", ge=1, le=20
    )
    max_players: int = Field(
        ..., description="Максимальное количество игроков", ge=1, le=50
    )
    play_time_minutes: Optional[int] = Field(
        None, description="Продолжительность игры в минутах", ge=1
    )
    age_rating: Optional[int] = Field(
        None, description="Возрастное ограничение", ge=0, le=18
    )
    ...
    total_copies: int = Field(
        ..., description="Общее количество экземпляров игры", ge=1
    )
...
class UpdateGameInfoRequest(BaseModel):
    ...
    name: Optional[str] = Field(
        None, description="Название игры", min_length=1, max_length=200
    )
    description: Optional[str] = Field(
        None, description="Описание игры", max_length=2000
    )
    min_players: Optional[int] = Field(
        None, description="Минимальное количество игроков", ge=1, le=20
    )
    max_players: Optional[int] = Field(
        None, description="Максимальное количество игроков", ge=1, le=50
    )
    play_time_minutes: Optional[int] = Field(
        None, description="Продолжительность игры в минутах", ge=1
    )
    ...
    category: Optional[str] = Field(None, description="Категория игры", max_length=100)
    price_per_day: Optional[float] = Field(
        None, description="Стоимость аренды за день", ge=0
    )
...
class UploadGamePhotosRequest(BaseModel):
    ...
    photo_urls: List[HttpUrl] = Field(
        ..., description="Список URL фотографий игры", min_items=1, max_items=10
    )
...
class UpdateAvailableGamesRequest(BaseModel):
    ...
    available_count: int = Field(
        ..., description="Количество доступных экземпляров игры", ge=0
    )
...
class FindGameRequest(BaseModel):
    ...
    category: Optional[str] = Field(
        None, description="Фильтр по категории", max_length=100
    )
    min_players: Optional[int] = Field(
        None, description="Минимальное количество игроков", ge=1
    )
    max_players: Optional[int] = Field(
        None, description="Максимальное количество игроков", ge=1
    )
    max_price: Optional[float] = Field(
        None, description="Максимальная стоимость аренды за день", ge=0
    )
...
class GameResponse(BaseModel):
    ...
    photo_urls: List[str] = Field(
        default_factory=list, description="Список URL фотографий"
    )
    rating: Optional[float] = Field(
        None, description="Средний рейтинг игры", ge=0, le=5
    )
...
class LeaveCommentRequest(BaseModel):
    ...
    comment_text: str = Field(
        ..., description="Текст комментария", min_length=1, max_length=2000
    )
...
class RegisterUserRequest(BaseModel):
    ...
    first_name: str = Field(
        ..., description="Имя пользователя", min_length=1, max_length=100
    )
    last_name: str = Field(
        ..., description="Фамилия пользователя", min_length=1, max_length=100
    )
...
class BlockUserRequest(BaseModel):
    ...
    reason: Optional[str] = Field(
        None, description="Причина блокировки", max_length=500
    )
...
class UpdateUserProfileRequest(BaseModel):
    ...
    first_name: Optional[str] = Field(
        None, description="Имя пользователя", min_length=1, max_length=100
    )
    last_name: Optional[str] = Field(
        None, description="Фамилия пользователя", min_length=1, max_length=100
    )
    phone: Optional[str] = Field(None, description="Номер телефона", max_length=20)
...
class CreateOrderRequest(BaseModel):
    ...
    booking_id: str = Field(
        ..., description="Идентификатор подтвержденного бронирования"
    )
    user_id: str = Field(..., description="Идентификатор пользователя")
    pickup_location: str = Field(
        ..., description="Адрес места самовывоза", max_length=200
    )
...
class ExtendRentalPeriodRequest(BaseModel):
    ...
    additional_days: int = Field(
        ..., description="Количество дополнительных дней аренды", ge=1, le=7
    )
...
class ReturnGameRequest(BaseModel):
    ...
    order_id: str = Field(..., description="Идентификатор заказа")
    user_id: str = Field(..., description="Идентификатор пользователя")
    return_location: str = Field(
        ..., description="Адрес места возврата", max_length=200
    )
...
class ConfirmGameReturnRequest(BaseModel):
    ...
    order_id: str = Field(..., description="Идентификатор заказа")
    game_condition: str = Field(
        ..., description="Состояние игры при возврате", max_length=200
    )
...
class ProcessPaymentRequest(BaseModel):
    ...
    transaction_id: str = Field(
        ..., description="Идентификатор транзакции от платежной системы"
    )
...
class RequestRefundRequest(BaseModel):
    ...
    amount: Optional[float] = Field(
        None, description="Сумма возврата (если частичный возврат)", ge=0
    )
...
class ProcessRefundRequest(BaseModel):
    ...
    transaction_id: str = Field(
        ..., description="Идентификатор транзакции возврата от платежной системы"
    )
...
class PaymentResponse(BaseModel):
    ...
    completed_at: Optional[datetime] = Field(
        None, description="Дата завершения платежа"
    )
...
class RefundResponse(BaseModel):
    ...
    transaction_id: Optional[str] = Field(
        None, description="Идентификатор транзакции возврата"
    )
    created_at: datetime = Field(..., description="Дата создания запроса на возврат")
    completed_at: Optional[datetime] = Field(
        None, description="Дата завершения возврата"
    )
