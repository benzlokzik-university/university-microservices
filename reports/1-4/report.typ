...
#show: gost.with(
  ...
  city: "Москва",
  performers: (
    (name: "Студент 1", position: "студент группы ИКБО-12-22"),
    (name: "Студент 2", position: "студент группы ИКБО-12-22"),
  ),
)
...
#figure(
  table(
    ...
    [Студент 1], [Аккаунт пользователя\ Каталог игр\ Бронирование],
    [Студент 2], [Аренда\ Оплата\ Отзывы],
  ),
  caption: [Распределение обязанностей],
) <table-distribution>

#link("https://unidraw.io/app/board/3d10426b651698bfc4c8?allow_guest=true")[Ссылка на доску].
...
Изучение способов коммуникации между сервисами, а также получение навыков принятия решений о дальнейших способах разработки сервисов.

== Ход работы

Необходимо разбить контексты по типам. Для этого обычно используется оценка сервиса по двум параметрам: конкурентные преимущества и сложность/стоимость разработки.

Для оценки сервисов используются три категории:

#list(
  [*S* — слабое влияние/низкая сложность реализации;],
  [*M* — среднее влияние/средняя сложность реализации;],
  [*L* — высокое влияние/высокая сложность реализации.],
)

Далее необходимо определить наличие коммуникаций между сервисами системы. Между контекстами может быть синхронный или асинхронный стиль общения.

На рисунке @fig-task4-1 показаны связи между сервисами.

#figure(
  image("task4/full graph.png", width: 100%),
  caption: [Межпроцессное взаимодействие в сервисах],
) <fig-task4-1>

== Вывод

В ходе выполнения практической работы были изучены способы коммуникации между сервисами, а также получены навыки принятия решений о дальнейших способах разработки сервисов. Были определены типы взаимодействия между микросервисами системы аренды настольных игр.
